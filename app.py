...
#Search Operation: Search transaction form
...
def search_transactions():
    ...
    # Creating a new list that contains only the transactions whose amount falls within the specified range. 
    minamount = float(request.form['min_amount'])
    maxamount = float(request.form['max_amount'])
    filtered_transactions = []

    for transaction in transactions:
        if transaction['amount'] >= minamount and \
        transaction['amount'] <= maxamount:
            filtered_transactions.append(transaction)
    return render_template('transactions.html', transactions=filtered_transactions)


#Total Balance: Displaying total balance
@app.route("/balance")
def total_balance():
    balance = 0
